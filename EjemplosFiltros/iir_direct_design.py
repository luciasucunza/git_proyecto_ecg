...
plt.figure()
w, mag, _ = my_analog_filter.bode()
plt.semilogx(w, mag, label=my_analog_filter_desc)
w, mag, _ = my_digital_filter.bode()
...
if filter_type == 'lowpass':
    ...
    plt.fill( [fstop_start, fstop_end,   fstop_end, fstop_start], [-attenuation, -attenuation, ymax, ymax], '0.9', lw=0) # stop
    plt.fill( [fpass_start, fpass_start, fpass_end, fpass_end],   [ymin, -ripple, -ripple, ymin], '0.9', lw=0) # pass
    ...
    plt.fill( [fstop_start, fstop_end,   fstop_end, fstop_start], [-attenuation, -attenuation, ymax, ymax], '0.9', lw=0) # stop
    plt.fill( [fpass_start, fpass_start, fpass_end, fpass_end],   [ymin, -ripple, -ripple, ymin], '0.9', lw=0) # pass
    plt.fill( [fstop2_start, fstop2_end,   fstop2_end, fstop2_start], [-attenuation, -attenuation, ymax, ymax], '0.9', lw=0) # stop
    ...
    plt.fill([fpass_start, fpass_start, fpass_end, fpass_end],   [ymin, -ripple, -ripple, ymin], '0.9', lw=0) # pass
    plt.fill([fstop_start, fstop_end,   fstop_end, fstop_start], [-attenuation, -attenuation, ymax, ymax], '0.9', lw=0) # stop
    plt.fill([fpass2_start, fpass2_start, fpass2_end, fpass2_end],   [ymin, -ripple, -ripple, ymin], '0.9', lw=0) # pass


plt.fill([xmin, xmin, fs/2, fs/2],   [ymin, -attenuation, -attenuation, ymin], mpl.colors.to_rgba('r', 0.3) , lw=1) # pass
